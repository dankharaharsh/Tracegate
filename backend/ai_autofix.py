...
def discover_repository_sources(
    ...
    if any(k in cwe for k in ["640", "330", "338", "613", "644", "116", "804"]) or any(k in corpus for k in ["password reset", "reset_token", "reset token", "recovery", "reset-password"]):
        cwe_concepts.update(["passwordreset", "reset_token", "reset", "recovery", "token", "password", "forgot"])
    ...
    for item in tree:
        ...
        has_symbol_endpoint = any(
            ...
            for s in symbols
        ) if symbols and endpoint_tokens else False
        has_endpoint_relevance = bool(matched_endpoint_tokens or has_symbol_endpoint or not endpoint)

        if base_p in CENTRAL_APP_ENTRYPOINTS and has_endpoint_relevance and (is_server_side or component.startswith("/") or "api" in corpus):
            score += 35
        ...
        if content:
            ...
            if allow_content_bonuses and (any(k in cwe for k in ["640", "330", "338", "613", "644"]) or "reset" in corpus) and any(k in lower_content for k in ["passwordreset", "reset_token", "reset_password", "generateresettoken", "complete_password_reset"]):
                score += 35
    ...
    score_threshold = max(50, max_score - 40)
    for c in candidates:
...
def detect_and_prune_duplicate_blocks(source_code: str, file_path: str = "") -> PruneResult:
    ...
    from source code across languages (JavaScript, TypeScript, Python, HTML, etc.).
    Preserves valid application structure while guaranteeing no defensive guard is duplicated.
    ...
    # Pass 2: Scope-level guard deduplication for JavaScript/TypeScript IDOR (userController.js)
    ...
    if len(guards) > 1:
        # Keep only the first authoritative guard; prune subsequent redundant blocks
        rebuilt = result_code[:guards[1].start()] + result_code[guards[1].end():]
        result_code = rebuilt
        was_pruned = True

    # Pass 3: Scope-level guard deduplication for Python 2FA blocks in app.py
