...
def discover_repository_sources(
    ...
    for item in tree:
        ...
        has_endpoint_relevance = bool(matched_endpoint_tokens or not endpoint)
        ...
        # Targeted content retrieval: fetch content only for candidate files
        is_content_candidate = (
            ...
            (layer in {"config", "settings"} and any(k in cwe for k in ["798", "312", "259"]))
        )
        ...
        if base_p in CENTRAL_APP_ENTRYPOINTS and not has_endpoint_relevance and symbols and endpoint_tokens:
