...
def auth_forgot_password(data: ForgotPasswordRequest):
    ...
    if res.get("status") == "ok":
        ...
        if not delivered:
            ...
            try:
                c = conn.cursor()
                c.execute("UPDATE password_resets SET consumed_at = datetime('now') WHERE id = ?", (res["reset_id"],))
                conn.commit()
            finally:
                conn.close()
            raise HTTPException(
    ...
    if getattr(req, "create_pr", True):
        try:
            ...
            if pr_res and pr_res.pr_url:
                ...
                cursor = conn.cursor()
                cursor.execute(
                    "UPDATE ai_fixes SET pr_url = ?, pr_number = ?, status = 'PR_CREATED' WHERE commit_sha = ? OR id = ?",
                    (pr_res.pr_url, pr_res.pr_number, res.commit_sha, res.fix_id)
                )
                conn.commit()
                conn.close()
