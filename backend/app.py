...
def auth_forgot_password(data: ForgotPasswordRequest):
    ...
    res = create_password_reset(data.email)
    if not res:
        # Generic message to protect privacy and prevent account enumeration
        return PasswordResetResponse(
            message="If this email is registered in Tracegate, password reset instructions have been generated.",
            reset_token=None
        )
    token, email = res
...
def download_project_report(
    ...
    """Download report file in Microsoft Word (.docx) or native PDF (.pdf) format."""
    if not authorization and token:
        authorization = f"Bearer {token}"
    user = get_current_authenticated_user(authorization)
    authorize_project(project_id, user)
    ...
    if fmt == "pdf":
        ...
        if not pdf_path.exists():
            compiled = convert_docx_to_pdf(str(docx_path))
...
@app.post("/api/ai-fix/scan-repository", response_model=RepositoryScanResponse)
def ai_fix_scan_repository_endpoint(req: RepositoryScanRequest, authorization: Optional[str] = Header(None)):
    user_id = _resolve_user_id(authorization)
    repo_val = req.repo or req.repository or "tracegate-lab/ecommerce-platform"
    branch_val = req.branch or "main"
    ...
    detected_list = scan_res.get("findings", [])
    ...
    if not target_project_id:
        ...
        conn.close()
        if prow:
            target_project_id = prow["id"]

    if req.auto_register and target_project_id:
        existing_project_findings = get_project_findings_list(target_project_id)
        existing_cwes = {f.get("cwe") for f in existing_project_findings if f.get("cwe")}
        existing_names = {(f.get("finding_name") or f.get("title") or "").lower() for f in existing_project_findings}

        for idx, df in enumerate(detected_list):
            is_dup = (df.get("cwe") in existing_cwes) or (df.get("finding_name", "").lower() in existing_names)
            if not is_dup:
                finding_data = {
                    "finding_name": df["finding_name"],
                    "title": df.get("title") or df["finding_name"],
                    "description": df["description"],
                    "observation": df["description"],
                    "cwe": df["cwe"],
                    "priority": normalize_severity(df.get("severity") or "HIGH"),
                    "cvss_score": df["cvss_score"],
                    "affected_component": df["file_path"],
                    "affected_endpoint": f"{df['file_path']}#L{df.get('line_number') or 1}",
                    "poc_text": df.get("poc_text", ""),
                    "remediation": df["remediation"],
                    "mitigation": df.get("mitigation", ""),
                    "source": "REPO_SCAN",
                    "status": "CONFIRMED"
                }
                saved = db_save_finding(target_project_id, None, finding_data)
                df["id"] = saved.get("id")
                df["vuln_id"] = saved.get("vuln_id") or df["vuln_id"]
            else:
                matched = next((f for f in existing_project_findings if f.get("cwe") == df.get("cwe")), None)
                if matched:
                    df["id"] = matched.get("id")
                    df["vuln_id"] = matched.get("vuln_id") or df["vuln_id"]

    return RepositoryScanResponse(
        success=True,
        repository=repo_val,
        branch=branch_val,
        scanned_files_count=scan_res.get("scanned_files_count", 0),
        vulnerabilities_detected_count=len(detected_list),
        findings=detected_list,
        summary=scan_res.get("summary", "Repository scan completed.")
    )
...
def ai_fix_batch_patch_endpoint(req: BatchPatchRequest, authorization: Optional[str] = Header(None)):
    ...
    def fetch_content_cb(fpath: str) -> str:
        if fpath not in file_cache:
            try:
                f_resp = get_file_contents(user_id, repo_val, branch_val, fpath)
                file_cache[fpath] = f_resp.get("content", "")
            except Exception as exc:
    ...
    if req.project_id:
        ...
        if req.finding_ids:
            ...
            findings_to_patch = [f for f in all_proj_findings if f.get("id") in target_ids or f.get("vuln_id") in target_ids]
        else:
...
def ai_fix_reject_endpoint(req: AIFixRejectRequest, authorization: Optional[str] = Header(None)):
    ...
    if latest_fix:
        ...
        save_ai_fix_record({
            "project_id": finding.get("project_id") or "proj-default",
            "finding_id": finding.get("id") or vuln_id,
            "repository": req.repo or req.repository or "tracegate-lab/ecommerce-platform",
            "base_branch": "main",
            "fix_branch": f"tracegate/fix/{vuln_id}",
            "file_path": "n/a",
            "status": "REJECTED",
...
def ai_fix_create_pr_endpoint(req: GitHubCreatePRRequest, authorization: Optional[str] = Header(None)):
    ...
    if is_batch:
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("UPDATE findings SET fix_status = 'PR Created', github_repo = ?, github_branch = ?, github_pr = ? WHERE status != 'RESOLVED'",
...
def get_ai_fix_endpoint(
    ...
    auth_id = finding.get("id") or clean_id
    ai_fix = get_ai_fix_for_finding(auth_id)
    if not ai_fix and auth_id != clean_id:
        ai_fix = get_ai_fix_for_finding(clean_id)
    if not ai_fix:
        return None
    return ai_fix
...
def project_certificate_status_endpoint(
    ...
    Returns status, remaining findings breakdown, and existing certificate if issued.
    """
    user = get_current_authenticated_user(authorization)
    authorize_project(project_id, user)
    return check_assessment_certificate_eligibility(project_id)
...
def download_certificate_endpoint(
    ...
    if fmt == "pdf":
        ...
        if pdf_path and Path(pdf_path).exists():
            return FileResponse(path=str(pdf_path), media_type="application/pdf", filename=Path(pdf_path).name)
        ...
        if docx_path and Path(docx_path).exists():
            ...
            if compiled and Path(compiled).exists():
                update_certificate_file_paths(certificate_id, pdf_path=compiled)
                return FileResponse(path=str(compiled), media_type="application/pdf", filename=Path(compiled).name)
        # Fallback to DOCX
        if docx_path and Path(docx_path).exists():
            return FileResponse(
                path=str(docx_path),
        ...
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Certificate document file not found on disk.")
    else:
        docx_path = cert.get("file_path_docx")
