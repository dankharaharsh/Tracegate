...
def get_repository_tree(user_id: str, repo: str, branch: str = "main") -> Dict[str, Any]:
    ...
    if repo and "/" in repo and not repo.lower().startswith("tracegate-lab/"):
        ...
        # If fetch fails for custom repo, return empty tree rather than falling back to demo files
        return {"repo": repo, "branch": branch, "tree": [], "truncated": False, "error": "Repository tree not accessible"}

    # Return mock file tree for built-in simulation tracegate-lab/* repositories
    if repo.lower().startswith("tracegate-lab/"):
        files_map = MOCK_REPO_FILES.get(repo, MOCK_REPO_FILES.get("tracegate-lab/ecommerce-platform", {}))
        tree_items = []
...
def get_file_contents(user_id: str, repo: str, branch: str, file_path: str) -> Dict[str, Any]:
    ...
    # 0. If this file is available in local branch store or mock files for tracegate-lab/*, return immediately
    if not _is_live_repo(repo, config) and repo.lower().startswith("tracegate-lab/"):
...
def analyze_finding_code(
    ...
    def content_fetcher(p: str) -> str:
        ...
        if clean_p in file_contents_map:
            return file_contents_map[clean_p]
        if p in file_contents_map:
            return file_contents_map[p]
        try:
    ...
    def run_single_file(single_path, snip=None):
        ...
        if not src and single_path:
            clean_sp = single_path.replace("\\", "/").lstrip("/")
            if clean_sp in file_contents_map:
                src = file_contents_map[clean_sp]
            elif single_path in file_contents_map:
                src = file_contents_map[single_path]
            else:
    ...
    should_fallback = (
        ...
        not file_contents_map and
        fix_dict.get("patch_status") != "ALREADY_SECURE" and
