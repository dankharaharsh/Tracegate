...
def get_file_contents(user_id: str, repo: str, branch: str, file_path: str) -> Dict[str, Any]:
    ...
    config = get_user_github_config(user_id)
    # 0. In mock/sandbox mode, if this file is available in local branch store or mock files, return immediately
    if not _is_live_repo(repo, config):
...
def create_finding_pull_request(
    ...
    # Sandbox PR assignment (strictly for built-in tracegate-lab local simulation)
...
def analyze_finding_code(
    ...
    should_fallback = (
        ...
        (
            fix_dict.get("patch_status") in ("EMPTY_PATCH", "PATCH_REJECTED", "NO_RELEVANT_SOURCE_FOUND", "NO_CHANGE_GENERATED") or
    ...
    if "remediation_summary" not in fix_dict or not fix_dict["remediation_summary"]:
        files_list = fix_dict.get("files") or []
        if not files_list and fix_dict.get("file_path"):
            files_list = [{
        ...
        fix_dict["remediation_summary"] = {
            ...
            "scanned_files_count": len(files_list) or 1,
            "relevant_files_count": len(files_list),
            "files_modified_count": len(files_list),
            "findings_validated_count": 1 if fix_dict.get("patch_status") == "PATCH_VALIDATED" else 0,
            "review_required_count": 0 if fix_dict.get("patch_status") == "PATCH_VALIDATED" else 1,
            "overall_status": "ALL_FINDINGS_VALIDATED" if fix_dict.get("patch_status") == "PATCH_VALIDATED" else "REVIEW_REQUIRED",
        ...
        if "finding_traceability" not in fix_dict or not fix_dict["finding_traceability"]:
            fix_dict["finding_traceability"] = [{
                ...
                "candidate_files": [f["path"] for f in files_list],
                "selected_files": [f["path"] for f in files_list],
                "selected_symbols": [],
                "change_plan": f"Remediate {finding.get('cwe') or 'vulnerability'} in {fix_dict.get('file_path', 'source')}",
                "changed_files": [f["path"] for f in files_list],
                "status": "PATCH_VALIDATED" if fix_dict.get("patch_status") == "PATCH_VALIDATED" else "REVIEW_REQUIRED",
                "validation": {
                    "source_found": "YES" if files_list else "NO",
                    "patch_generated": "YES" if fix_dict.get("diff_unified") else "NO",
                    "patch_validated": "YES" if fix_dict.get("patch_status") == "PATCH_VALIDATED" else "NO"
                },
                "reason": fix_dict.get("reason") or fix_dict.get("explanation") or "Patch validated cleanly."
            }]
