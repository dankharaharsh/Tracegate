...
def map_finding_to_code(
    ...
    if is_package_dep:
        ...
        for hint in [f_path, comp]:
            if hint and (hint in index.files or index.get_content(hint)) and hint not in selected_files:
...
def apply_semantic_remediations_to_file(
    ...
    for intent in norm_intents:
        ...
        # -------------------------------------------------------------
        if allow_autofix_fallback and modified_code == code_before_intent:
            try:
