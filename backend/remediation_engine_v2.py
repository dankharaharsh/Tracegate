...
def map_finding_to_code(
    ...
    if is_package_dep:
        ...
        for hint in [f_path, comp]:
            clean_hint = hint.replace("\\", "/").lstrip("/") if hint else ""
            if clean_hint and clean_hint in index.files and clean_hint not in selected_files:
                selected_files.insert(0, clean_hint)
            elif clean_hint and "/" in clean_hint:
                base_h = os.path.basename(clean_hint)
                for item in tree_items:
                    ip = item.get("path", "").replace("\\", "/").lstrip("/")
                    if (ip == clean_hint or os.path.basename(ip) == base_h) and ip in index.files and ip not in selected_files:
