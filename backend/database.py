...
def get_finding_scoped(
    ...
    if project_id:
        ...
        LEFT JOIN checklist_items ci ON f.checklist_item_id = ci.id
        WHERE f.id = ? AND (p.owner_id = ? OR p.owner_id IS NULL OR p.owner_id IN ('user-learner-001', 'Security Learner'))
        LIMIT 1
        """, (finding_id, uid))
        row = cursor.fetchone()
        if not row:
            ...
            JOIN projects p ON f.project_id = p.id
            LEFT JOIN checklist_items ci ON f.checklist_item_id = ci.id
            WHERE f.vuln_id = ? AND (p.owner_id = ? OR p.owner_id IS NULL OR p.owner_id IN ('user-learner-001', 'Security Learner'))
            ORDER BY f.recorded_at DESC
            LIMIT 1
            """, (finding_id, uid))
            row = cursor.fetchone()
        ...
        JOIN projects p ON f.project_id = p.id
        LEFT JOIN checklist_items ci ON f.checklist_item_id = ci.id
        WHERE f.id = ? AND (p.owner_id IS NULL OR p.owner_id IN ('user-learner-001', 'Security Learner'))
        LIMIT 1
        """, (finding_id,))
        row = cursor.fetchone()
        if not row:
            ...
            JOIN projects p ON f.project_id = p.id
            LEFT JOIN checklist_items ci ON f.checklist_item_id = ci.id
            WHERE f.vuln_id = ? AND (p.owner_id IS NULL OR p.owner_id IN ('user-learner-001', 'Security Learner'))
            ORDER BY f.recorded_at DESC
            LIMIT 1
            """, (finding_id,))
            row = cursor.fetchone()
