...
def init_db():
    ...
    # Data migration: assign unowned legacy development projects to primary developer user 'user-1789554357515' (harsh123@gmail.com)
    cursor.execute("UPDATE projects SET owner_id = 'user-1789554357515' WHERE owner_id IS NULL OR owner_id = ''")
    ...
    for idx, itm in enumerate(evidence_items):
        ...
        try:
            ...
            INSERT OR REPLACE INTO evidence (
                ...
                caption, description, uploaded_by, uploaded_at, status
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                ev_id,
                ev_ref,
...
def save_user_github_config(user_id: str, token: Optional[str], mode: str = "mock", username: Optional[str] = None):
    ...
    if user_id != "usr-learner-001":
        cursor.execute("""
        INSERT INTO user_github_configs (user_id, token, mode, username, updated_at)
        VALUES ('usr-learner-001', ?, ?, ?, ?)
        ON CONFLICT(user_id) DO UPDATE SET
            token = excluded.token,
            mode = excluded.mode,
            username = excluded.username,
            updated_at = excluded.updated_at
        """, (token, mode, username, now))
    conn.commit()
    conn.close()
...
def get_user_github_config(user_id: str) -> Optional[Dict[str, Any]]:
    ...
    row = cursor.fetchone()
    if not row and user_id and user_id != "usr-learner-001":
        cursor.execute("SELECT * FROM user_github_configs WHERE user_id = 'usr-learner-001'")
        row = cursor.fetchone()
    if not row:
        cursor.execute("SELECT * FROM user_github_configs WHERE mode = 'live' AND token IS NOT NULL ORDER BY updated_at DESC LIMIT 1")
        row = cursor.fetchone()
    conn.close()
...
def save_report_record(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Check if file_path_pdf column exists in reports
    cursor.execute("PRAGMA table_info(reports)")
    cols = [r["name"] for r in cursor.fetchall()]
    if "file_path_pdf" in cols:
        cursor.execute("""
        INSERT INTO reports (
            id, project_id, version, report_title, file_path, file_path_pdf,
