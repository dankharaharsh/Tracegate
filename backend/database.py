...
def save_source_discovery(data: Optional[Dict[str, Any]] = None, **kwargs) -> Dict[str, Any]:
    ...
    if project_id and repository:
        cursor.execute("SELECT id FROM source_discovery_results WHERE finding_id = ? AND (project_id = ? OR repository = ?)", (finding_id, project_id, repository))
    elif project_id:
        cursor.execute("SELECT id FROM source_discovery_results WHERE finding_id = ? AND (project_id = ? OR repository = ?)", (finding_id, project_id, project_id))
    elif repository:
        cursor.execute("SELECT id FROM source_discovery_results WHERE finding_id = ? AND (project_id = ? OR repository = ?)", (finding_id, repository, repository))
    else:
...
def get_source_discovery(finding_id: str, project_id: Optional[str] = None, repo: Optional[str] = None) -> Optional[Dict[str, Any]]:
    conn = get_db_connection()
    cursor = conn.cursor()
    target_scope = repo or project_id
    if repo and project_id and repo != project_id:
        cursor.execute("SELECT * FROM source_discovery_results WHERE finding_id = ? AND (project_id = ? OR repository = ?)", (finding_id, project_id, repo))
    elif target_scope:
        cursor.execute("SELECT * FROM source_discovery_results WHERE finding_id = ? AND (project_id = ? OR repository = ?)", (finding_id, target_scope, target_scope))
    else:
