...
def run_all_auth_tests():
    ...
    # TEST 15: Forgot Password Dispatches OTP & Does NOT Expose Token
    print("\n[TEST 15] Forgot Password Dispatches OTP & Does NOT Expose Token...")
    ...
    assert "reset_token" not in forgot_data or forgot_data.get("reset_token") is None, "Secret reset_token must NOT be in API response!"
    assert "verification code has been sent" in forgot_data.get("message", "").lower()

    otp = email_service.get_last_otp_for_email(test_email)
    assert otp is not None and len(otp) == 6 and otp.isdigit(), f"Expected 6-digit OTP in outbox, got {otp}"
    print(f"  [PASS] Generic response returned; 6-digit OTP dispatched to outbox: {otp[:2]}****")

    # TEST 15b: Verify OTP & Issue Reset Authorization
    print("\n[TEST 15b] Verify OTP & Issue Reset Authorization...")
    verify_resp = client.post("/api/auth/verify-reset-otp", json={"email": test_email, "otp": otp})
    assert verify_resp.status_code == 200, f"Expected 200, got {verify_resp.status_code}: {verify_resp.text}"
    reset_auth = verify_resp.json().get("reset_authorization")
    assert reset_auth is not None and reset_auth.startswith("rst-auth-")
    print(f"  [PASS] OTP verified; single-use reset authorization issued: {reset_auth[:18]}...")
    ...
    print("  [PASS] Single-use enforcement confirmed: used reset authorization rejected.")

    # TEST 18: Expired Reset Token Rejection
    print("\n[TEST 18] Expired Reset Token Rejection...")
    conn_cleanup = get_db_connection()
    conn_cleanup.execute("UPDATE password_resets SET created_at = datetime('now', '-2 hours') WHERE user_id = (SELECT id FROM users WHERE email = ?)", (test_email,))
    conn_cleanup.commit()
    conn_cleanup.close()

    forgot2 = client.post("/api/auth/forgot-password", json={"email": test_email})
    otp2 = email_service.get_last_otp_for_email(test_email)
    v2_resp = client.post("/api/auth/verify-reset-otp", json={"email": test_email, "otp": otp2})
    tok2 = v2_resp.json()["reset_authorization"]
    ...
    c_exp.execute("UPDATE password_resets SET expires_at = ? WHERE reset_token_hash = ?", (past_time, tok2_hash))
    conn_exp.commit()
    conn_exp.close()

    expired_resp = client.post("/api/auth/reset-password", json={
        "reset_authorization": tok2,
