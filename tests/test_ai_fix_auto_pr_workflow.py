...
class TestAIFixAutoPRWorkflow(unittest.TestCase):
    ...
    def test_03_github_config_fallback_resolution(self):
        """3. get_user_github_config resolves to default/learner token when user row is absent."""
        # Save config under default learner ID
        save_user_github_config("usr-learner-001", "ghp_learnerToken1234567890", "live", "learner-git")

        # Query config for an arbitrary unconfigured user
        cfg = get_user_github_config(f"unconfigured-user-{self.ts}")
        self.assertIsNotNone(cfg)
        self.assertEqual(cfg["username"], "learner-git")
        self.assertEqual(cfg["token"], "ghp_learnerToken1234567890")
