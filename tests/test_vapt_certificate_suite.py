...
class TestVAPTCertificateSuite(unittest.TestCase):
    ...
    def test_10_manual_retest_route_triggers_certificate(self):
        """Retest via /api/findings/{finding_id}/retest route alias also triggers certificate evaluation."""
        f1 = self._create_finding("Sensitive Data Exposure", severity="MEDIUM", status="CONFIRMED", retest_status="PENDING")
        res = client.post(f"/api/findings/{f1}/retest", json={
            "result": "PASS",
            "notes": "Verified patch manually"
        })
        self.assertEqual(res.status_code, 200)
        data = res.json()
        ...
        self.assertIn("TG-VAPT-", data["certificate"]["certificate_id"])

    def test_11_multi_finding_assessment_with_blocking_reason(self):
        """Retesting 1 of 3 findings returns eligible=False and explicit blocking reason."""
        f1 = self._create_finding("SQL Injection", severity="CRITICAL", status="CONFIRMED", retest_status="PENDING")
        f2 = self._create_finding("XSS Stored", severity="HIGH", status="CONFIRMED", retest_status="PENDING")
        f3 = self._create_finding("CSRF", severity="LOW", status="CONFIRMED", retest_status="PENDING")

        res = client.post(f"/api/ai-fix/{f1}/retest", json={
            "result": "PASS",
            "notes": "SQLi parameterization verified"
        })
        self.assertEqual(res.status_code, 200)
        data = res.json()
        self.assertEqual(data["status"], "RESOLVED")
        ...
        self.assertIsNotNone(data.get("blocking_reason"))
        self.assertIn("2 in-scope finding(s) remain pending retest", data["blocking_reason"])

    def test_12_status_endpoint_returns_diagnostic_fields(self):
        """Status endpoint returns comprehensive diagnostic fields for development and UI display."""
        f1 = self._create_finding("Broken Auth", severity="HIGH", status="RESOLVED", retest_status="PASSED")
        f2 = self._create_finding("Path Traversal", severity="HIGH", status="CONFIRMED", retest_status="PENDING")

        res = client.get(f"/api/projects/{self.proj_id}/certificate/status")
        self.assertEqual(res.status_code, 200)
        data = res.json()
        self.assertEqual(data["assessment_id"], self.proj_id)
        self.assertEqual(data["total_findings"], 2)
        self.assertEqual(data["resolved_findings"], 1)
        self.assertEqual(data["passed_retests"], 1)
        self.assertEqual(data["pending_findings"], 1)
        self.assertEqual(data["pending_retests"], 1)
        self.assertEqual(data["failed_retests"], 0)
        self.assertFalse(data["eligible"])
        self.assertIn("blocking_reason", data)
        self.assertIn("remaining_by_severity", data)
