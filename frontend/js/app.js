...
`;
    ...
    async function openBrowseRepoTreeModal() {
        ...
        try {
            ...
            const validPaths = new Set(currentRepoTreeItems.map(item => item.path));
            repoTreeTempCheckedPaths = new Set((window.selectedSources || []).map(s => s.path).filter(p => validPaths.has(p)));
            updateRepoTreeCount();
    ...
    async function executeCodeFixAnalysis(repo, branch, findingId, isWorkspace = true) {
        ...
        try {
            ...
            const batchFindingIds = (curProj && Array.isArray(curProj.findings) && curProj.findings.length > 0)
                ? curProj.findings.map(f => f.id || f.vuln_id)
                : undefined;
    ...
    if (btnSelectRepoFileFromTree) {
        btnSelectRepoFileFromTree.addEventListener("click", async () => {
            const validPaths = new Set((currentRepoTreeItems || []).map(i => i.path));
            const chosen = Array.from(repoTreeTempCheckedPaths).filter(p => validPaths.has(p));
            if (chosen.length === 0) {
                showToast("Please select at least one file from the current repository.", "warning");
                return;
            }
    ...
    function resetRepositorySourceScope() {
        window.selectedSources = [];
        window.candidateSources = [];
        window.sourceSelectionMode = "AUTOMATIC";
        currentRepoTreeItems = [];
    ...
    function applyCustomRepository(customName) {
        ...
        resetRepositorySourceScope();
        loadRepositoryBranches(cleanRepo);
        const selFinding = document.getElementById("wsAutofixFindingSelect")?.value;
        if (selFinding && selFinding !== "__ALL_FINDINGS__") {
            restoreFindingFixState(selFinding);
        }
    ...
    async function restoreFindingFixState(fid) {
        ...
        try {
            ...
            const curProjId = curProj ? curProj.id : (typeof activeProjectId !== "undefined" ? activeProjectId : "");
            if (repoVal && fid) {
                const sUrl = `/api/ai-fix/selected-sources?finding_id=${encodeURIComponent(fid)}&repo=${encodeURIComponent(repoVal)}${curProjId ? `&project_id=${encodeURIComponent(curProjId)}` : ""}`;
