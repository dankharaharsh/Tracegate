...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function navigateTo(hash) {
        ...
        if (isPublicRoute) {
            ...
            if (isAuthRoute) {
                // If user is already authenticated and explicitly navigates to login/register, redirect to dashboard
    ...
    async function executeLogin() {
        ...
        try {
            ...
            // Load user-scoped projects
            const userProjCache = localStorage.getItem("tg_projects_" + currentUser.id);
            if (userProjCache) {
                try { projects = JSON.parse(userProjCache); if (!Array.isArray(projects)) projects = []; } catch (e) { projects = []; }
            } else {
                projects = [];
            }
            activeProjectId = localStorage.getItem("tg_active_proj_id_" + currentUser.id) || (projects[0]?.id || null);
    ...
    if (btnSignupSubmit) {
        btnSignupSubmit.addEventListener("click", async () => {
            ...
            try {
                ...
                // New user starts with empty project list
                projects = [];
                    ...
                    reports.forEach(r => {
                        rows += `
                            <tr>
                                ...
                                <td><span class="badge badge-completed">DOCX + PDF</span></td>
                                <td>
                                    <div style="display: flex; gap: 6px;">
                                        <a href="${getAuthDownloadUrl('/api/projects/' + encodeURIComponent(projId) + '/reports/' + encodeURIComponent(r.id) + '/download?format=docx')}" class="btn btn-outline-primary btn-sm" download title="Download Microsoft Word Document">
                                            📥 DOCX
                                        </a>
                                        <a href="${getAuthDownloadUrl('/api/projects/' + encodeURIComponent(projId) + '/reports/' + encodeURIComponent(r.id) + '/download?format=pdf')}" class="btn btn-outline-danger btn-sm" download title="Download Native PDF Document">
                                            📄 PDF
                                        </a>
                                    </div>
                                </td>
    ...
    // Dedicated explicit download helper for generated reports with double-click protection
    let isDownloadingReport = false;
    function handleExplicitReportDownload(projId, reportId, format, filename, triggerBtn) {
        ...
        try {
            const downloadUrl = getAuthDownloadUrl(`/api/projects/${encodeURIComponent(projId)}/reports/${encodeURIComponent(reportId)}/download?format=${encodeURIComponent(format)}`);
            const dlLink = document.createElement("a");
            dlLink.href = downloadUrl;
            if (filename) dlLink.download = filename;
            document.body.appendChild(dlLink);
            dlLink.click();
            document.body.removeChild(dlLink);
            showToast(`📥 Initiating ${format.toUpperCase()} download...`, "info");
        } catch (err) {
        ...
        btnGenerateDocxReport.addEventListener("click", async () => {
            ...
            try {
                ...
                setTimeout(() => {
                    ...
                    const btnDlPdf = document.getElementById("btnDownloadGeneratedPdf");
                    if (btnDlPdf) {
                        btnDlPdf.onclick = (e) => {
                            e.preventDefault();
                            const pdfName = (reportRecord.filename ? reportRecord.filename.replace(/\.docx$/i, ".pdf") : `${proj.name}_vapt_report_${version}.pdf`);
                            handleExplicitReportDownload(proj.id, reportRecord.id, "pdf", pdfName, btnDlPdf);
                        };
                    }
...
`;
    ...
    async function executeCodeFixAnalysis(repo, branch, findingId, isWorkspace = true) {
        ...
        try {
            ...
            if (data.file_traceability && data.file_traceability.length > 0 && fileTraceCard && fileTraceTbody) {
                fileTraceCard.style.display = "flex";
                if (fileTraceCountBadge) fileTraceCountBadge.textContent = `${data.file_traceability.length} file${data.file_traceability.length > 1 ? "s" : ""}`;
                fileTraceTbody.innerHTML = "";

                data.file_traceability.forEach(ft => {
                    const tr = document.createElement("tr");
                    tr.style.borderBottom = "1px solid var(--border-subtle)";
            ...
            try {
                ...
                if (!res.ok) {
                    ...
                    if (res.status === 401) {
                        ...
                        errorMsg = "Unable to record retest. Please try again.";
    ...
    function pollForCertificate(projectId, maxAttempts = 10, intervalMs = 1500) {
        if (!projectId) return;
        if (certPollingTimer) {
        ...
        certPollingTimer = setInterval(async () => {
            ...
            try {
                ...
                } else if (data.status === "FAILED" || (!data.eligible && attempts > 1) || attempts >= maxAttempts) {
                    clearInterval(certPollingTimer);
                    certPollingTimer = null;
                    refreshProjectCertificateStatus(projectId);
                }
            ...
            if (data.status === "GENERATED" && data.certificate) {
                ...
                if (contentArea) {
                    ...
                    document.getElementById("btnWsDownloadCert")?.addEventListener("click", () => {
                        if (typeof downloadCertificateFile === "function") {
                            downloadCertificateFile(cert.certificate_id, "pdf");
                        } else {
                            window.open(getAuthDownloadUrl(`/api/certificates/${encodeURIComponent(cert.certificate_id)}/download?format=pdf`), "_blank");
                        }
                    });
                }
                ...
                if (contentArea) {
                    ...
                    document.getElementById("btnRetryGenerateCert")?.addEventListener("click", async () => {
                        ...
                        try {
                            ...
                            const gRes = await fetch(`/api/projects/${projectId}/certificate/generate`, {
                                method: "POST",
                                headers: getAuthHeaders()
                            });
                            const gData = await gRes.json();
                            if (gRes.ok && gData.certificate) {
    ...
    async function downloadCertificateFile(certId, format = "pdf") {
        ...
        try {
            ...
            if (!res.ok) {
                ...
                const fallbackUrl = getAuthDownloadUrl(`/api/certificates/${encodeURIComponent(certId)}/download?format=${encodeURIComponent(fmt)}`);
    ...
    const btnDownloadCertDocxModal = document.getElementById("btnDownloadCertDocxModal");
    ...
    if (btnDownloadCertDocxModal) {
        btnDownloadCertDocxModal.addEventListener("click", () => {
            if (activeCertificateData && activeCertificateData.certificate_id) {
                downloadCertificateFile(activeCertificateData.certificate_id, "docx");
            }
        });
    }
    ...
    document.getElementById("btnWsAIFixDownloadDocx")?.addEventListener("click", () => handleAIFixDownloadCertificate("docx"));
