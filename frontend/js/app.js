...
`;
    ...
    function renderSelectedSourcesList() {
        ...
        if (modeBadge) {
            modeBadge.textContent = window.sourceSelectionMode === "USER_MODIFIED" ? "User Modified" : "✓ Entire Repository";
    ...
    async function executeCodeFixAnalysis(repo, branch, findingId, isWorkspace = true) {
        ...
        // For single-finding mode, respect user-selected sources in modal ONLY when explicitly modified by user.
        // If window.sourceSelectionMode !== "USER_MODIFIED", leave selected_files undefined and file_path null
        ...
        if (!isBatchAll && window.sourceSelectionMode === "USER_MODIFIED") {
