...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function clearUserSessionState() {
        ...
        const btnAutoDetect = document.getElementById("btnWsAutoDetectFile");
            ...
            if (res.ok) {
                ...
                if (Array.isArray(reports) && reports.length > 0) {
                    ...
                    reports.forEach(r => {
                        ...
                        const badgeHtml = hasPdf
                            ? `<span class="badge badge-completed">DOCX + PDF</span>`
                            : `<span class="badge badge-info">DOCX</span>`;
                        const safeDocxName = r.filename || `${r.project_id}_${r.version}.docx`;
                        const safePdfName = safeDocxName.replace(/\.docx$/i, ".pdf");
                        rows += `
                            <tr>
                                ...
                                <td>
                                    <div style="display: flex; gap: 6px;">
                                        <button type="button" class="btn btn-outline-info btn-sm btn-view-hist-pdf" data-proj="${escapeHtml(projId)}" data-rep="${escapeHtml(r.id)}" title="View PDF Report in Browser Tab">
                                            👁 View
                                        </button>
                                        ...
                                        <button type="button" class="btn btn-outline-danger btn-sm btn-dl-hist-pdf" data-proj="${escapeHtml(projId)}" data-rep="${escapeHtml(r.id)}" data-fn="${escapeHtml(safePdfName)}" title="Download Native PDF Document">
                                            📄 PDF
                                        </button>
                                    </div>
                                </td>
                    ...
                    // Wire inline PDF viewing in a new tab with proper document title
                    tbody.querySelectorAll(".btn-view-hist-pdf").forEach(btn => {
                        btn.addEventListener("click", (e) => {
                            e.preventDefault();
                            const token = localStorage.getItem("token") || "";
                            const url = `/api/projects/${encodeURIComponent(btn.dataset.proj)}/reports/${encodeURIComponent(btn.dataset.rep)}/view?token=${encodeURIComponent(token)}`;
                    ...
                    tbody.querySelectorAll(".btn-dl-hist-pdf").forEach(btn => {
                        btn.addEventListener("click", (e) => {
                            e.preventDefault();
                            handleExplicitReportDownload(btn.dataset.proj, btn.dataset.rep, "pdf", btn.dataset.fn, btn);
                        });
                    });
        ...
        btnGenerateDocxReport.addEventListener("click", async () => {
            ...
            try {
                ...
                setTimeout(() => {
                    ...
                    const btnViewPdf = document.getElementById("btnViewGeneratedPdf");
                    if (btnViewPdf) {
                        btnViewPdf.onclick = (e) => {
                            e.preventDefault();
                            const token = localStorage.getItem("token") || "";
                            const url = `/api/projects/${encodeURIComponent(proj.id)}/reports/${encodeURIComponent(reportRecord.id)}/view?token=${encodeURIComponent(token)}`;
                            window.open(url, "_blank");
                        };
                    }
...
`;
    ...
    async function autoDetectSourceFile() {
        if (!window._githubConnected) {
            showToast("GitHub is not connected. Connect GitHub to enable automatic source discovery.", "warning");
            return;
        }
    ...
    async function executeCodeFixAnalysis(repo, branch, findingId, isWorkspace = true) {
        ...
        // For single-finding mode, respect user-selected sources in modal.
        // For bulk "Remediate All" mode, default to undefined to trigger universal whole-repository dynamic discovery for each finding.
        let selectedPaths = [];
        ...
        try {
            ...
            // Configure Multi-File Patch View Tabs - Strictly filter to truly modified files
            ...
            if (filesList.length > 0) {
                ...
                if (filePathEl) filePathEl.textContent = data.file_path || "No Files Modified";
                if (diffContainer) {
                    diffContainer.innerHTML = `<div style="padding: 24px; text-align: center; color: var(--text-muted); font-size: 0.88rem;">${data.patch_status === 'ALREADY_SECURE' ? '✓ Finding is already securely remediated in repository source code. No changes required.' : 'No modified lines generated for this finding.'}</div>`;
                }
                if (beforeCodeContainer) beforeCodeContainer.textContent = data.before_code || data.original_code || "";
                if (afterCodeContainer) afterCodeContainer.textContent = data.after_code || data.proposed_code || "";
            }
